...
class ConfigTranslation(QObject):
    ...
    def __init__(self, parent=None):
        ...
        self.entries = {
            ...
            self.tr('设置失败'): '设置失败',
            self.tr('警告'): '警告',
            self.tr('设置可能会出现问题，当前值为：'): '设置可能会出现问题，当前值为：',
            self.tr('当前值为：'): '当前值为：',
            self.tr('摸头轮数'): '摸头轮数',
            self.tr('摸头轮数设置错误'): '摸头轮数设置错误',
            self.tr('请设置为4-15之间的整数'): '请设置为4-15之间的整数',
            self.tr('表单修改成功'): '表单修改成功',
            self.tr('已经被设置为：'): '已经被设置为：',
            self.tr('请检查输入的数据是否正确'): '请检查输入的数据是否正确',
        }
